...
AGN_safeZone_Backpack_AllowGearFromDeadPlayers = false;		//Should players be able to loot from a dead players corpse?
...
while {true} do {
	...
	if ( AGN_safeZone_Backpack_EnableAntiBackpack ) then
	{
		...
		AGN_LastPlayerLookedAtCountDown = 3;
		_antiBackpackThread = [] spawn {
			...
			while {!canBuild} do
			{
				if ( isNull AGN_LastPlayerLookedAt ) then
				{
					...
					if ( AGN_LastPlayerLookedAtCountDown < 0 ) then { AGN_LastPlayerLookedAtCountDown = 3; AGN_LastPlayerLookedAt = objNull; };
					sleep 1;
				};
			};
		};
